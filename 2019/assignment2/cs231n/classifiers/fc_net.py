...
class TwoLayerNet(object):
    ...
    def loss(self, X, y=None):
        ...
        scores = np.dot(X_copy,self.params['W1']) + self.params['b1']
        a1 = np.copy(scores)
        ...
        scores = np.exp(scores[range(N),y]) / np.sum(np.exp(scores),axis=1,keepdims=True)
        ...
        dp[range(N),y] -= 1
        dp /= N
        grads['W2'] = np.dot(a2.T,dp) + self.reg*(self.params['W2'])
        grads['b2'] = np.sum(dp,axis=0)
        da2 = np.dot(dp,self.params['W2'].T)
        da2 = da2*(a1>0)
        grads['W1'] = np.dot(X_copy.T,da2) + self.reg*(self.params['W1'])
    ...
    def loss(self, X, y=None):
        ...
        for i in range(1,self.num_layers+1) :
            ...
            if self.use_dropout : 
                ...
                cache['dropout_%s'%(i)] = cache_dropout
        ...
        for i in range(self.num_layers-1,0,-1) :
            if self.use_dropout : 
                dout = dropout_backward(dout,cache['dropout_%s'%(i)])
        ...
        # gradient for hidden & input layers
        for i in range(self.num_layers,0,-1) : 
#            if i != self.num_layers :  
            grads['W%s'%(i)] = np.dot(layers[i-1].T,dlast) + self.reg*self.params['W%s'%(i)]
